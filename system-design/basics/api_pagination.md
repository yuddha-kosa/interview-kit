...
## 1. API Pagination: Offset-Based vs. Cursor-Based
...
## 2. Offset-Based Pagination Deep Dive

### How it works (SQL Level)

The client passes a relative position marker using page or offset.

```sql
-- Fetching page 3 (items 21-30)
SELECT * FROM posts ORDER BY created_at DESC LIMIT 10 OFFSET 20;
```

### The Scalability Flaw (The O(N) Linear Scan)
...
## 3. Cursor-Based Pagination Deep Dive
...
```sql
-- Fetching the next 10 items AFTER post ID 452
SELECT * FROM posts WHERE id < 452 ORDER BY id DESC LIMIT 10;
```
...
## 4. Production-Grade API Design & Implementation

### The Industry-Standard JSON Payload Contract
...
## 5. Edge Cases & Advanced Cursor Patterns

### Problem: Non-Unique Sorting Keys (e.g., likes_count)
...
```sql
-- Query executed behind the scenes for: GET /posts?limit=10&before=1500
SELECT * FROM posts WHERE id > 1500 ORDER BY id ASC LIMIT 10;
-- Resulting array is reversed in Java/Go/Node.js memory before returning to user
```

---

## 6. Identity Generation: Auto-Incrementing IDs vs. UUIDv7
...
| Security / Obscurity | Poor. Sequentially guessable sequences expose total transactions via URL scraping. | Excellent. The trailing random bits make IDs unguessable. |
