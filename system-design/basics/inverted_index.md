...
| Term (Key) | Postings List (Value / Matching Doc IDs) |
|---|---|
| are | [2] |
| design | [1, 2, 3] |
| great | [2] |
...
* The Postings Lists: The list of Doc IDs (Values). These arrays grow massive. Engines use bitmask compression techniques (like Frame of Reference (FoR) or Roaring Bitmaps) to shrink the list of integers on disk and accelerate the mathematical intersection math during a search.

Would you like to explore how an inverted index handles phrase searches (knowing the exact order of words using positional data), or look into how Document Frequency (TF-IDF / BM25) is stored alongside the index to rank the best results first?
